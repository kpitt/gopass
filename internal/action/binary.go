...
func (s *Action) binaryCopy(ctx context.Context, c *cli.Context, from, to string, deleteSource bool) error {
	...
	case fsutil.IsFile(from) && fsutil.IsFile(to):
		...
		return fmt.Errorf("Cannot %s from file to file. Either source or destination must be a password store secret.", op)
	case s.Store.Exists(ctx, from) && s.Store.Exists(ctx, to):
		// copying from one secret to another secret is not supported.
		return fmt.Errorf("Cannot %s from secret to secret. Either source or destination must be a file.", op)
	case fsutil.IsFile(from) && !fsutil.IsFile(to):
	...
	default:
		return fmt.Errorf("Unhandled fs%s operation. Please report a bug.", op)
	}
}
...
func (s *Action) binaryValidate(ctx context.Context, buf []byte, name string) error {
	...
	if fileSum != storeSum {
		return fmt.Errorf("Content hashes do not match (file: %s, store: %s)", fileSum, storeSum)
	}
