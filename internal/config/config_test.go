...
func TestNewConfig(t *testing.T) { //nolint:paralleltest
	...
	cfg := config.New()
	cs := cfg.String()
	assert.Contains(t, cs, `&config.Config{AutoImport:false, ClipTimeout:45, ExportKeys:true, NoPager:false, Notifications:true,`)
	...
	cs = cfg.String()
	assert.Contains(t, cs, `&config.Config{AutoImport:false, ClipTimeout:0, ExportKeys:false, NoPager:false, Notifications:false,`)
